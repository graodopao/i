...
draw_text_transformed_color(display_get_gui_width() / 2 - (string_width((y < 60) ? next_letter : "- press space to retry -") / 2), display_get_gui_height() / 2 - string_height(next_letter), y < 60 ? next_letter : "- press space to retry -", 1, 1, 0, y < 60 ? c_white : c_black, y < 60 ? c_white : c_black, y < 60 ? c_white : c_black, y < 60 ? c_white : c_black, y < 60 ? 1 : (clamp(y * 0.02 - 1.2, 0, 1)));
if (keyboard_lastchar == next_letter and y < 60){
	y = 0};
if (y >= 60 and keyboard_check_pressed(vk_space)){
	game_restart()};
draw_text_transformed_color(display_get_gui_width() / 2 - (string_width("- " + string((keyboard_lastchar == next_letter and y < 60) ? x++ : x) + " -") / 2), string_height("- " + string(x) + " -"), "- " + string(x) + " -", 1, 1, 0, y < 60 ? c_white : c_black, y < 60 ? c_white : c_black, y < 60 ? c_white : c_black, y < 60 ? c_white : c_black, y < 60 ? 1 : (clamp(y * 0.02 - 1.2, 0, 1)));